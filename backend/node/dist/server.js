...
const client_1 = require("@prisma/client"); // <-- include Prisma enum + client
const prisma = new client_1.PrismaClient();
const app = (0, fastify_1.default)({ logger: true });
// ---- Contacts helpers ----
const toDigits = (v) => (v || '').replace(/\D/g, '');
const clamp10 = (d) => d.slice(0, 10);
// Map UI payload -> Prisma data
function uiToDbContact(body) {
    // required: name, phone
    ...
    if (phoneDigits.length !== 10)
        throw new Error('VALIDATION:phone must have 10 digits');
    const data = {
        contact_name: name,
...
async function start() {
    ...
    const getRentHandler = async (req, reply) => {
        const { property_id, year } = req.query;
        if (!property_id)
            return reply.status(400).send({ error: 'property_id required' });
        const where = { property_id: Number(property_id) };
        if (year)
    ...
    const postRentHandler = async (req, reply) => {
        const b = req.body;
    ...
    app.post('/api/transactions', async (req, reply) => {
        const { property_id, amount, date, transaction_type, notes } = req.body;
    ...
    app.patch('/api/transactions/:id', async (req, reply) => {
        ...
        if ('transaction_type' in body)
            updates.transaction_type = body.transaction_type ?? null;
        if ('notes' in body)
    ...
    app.get('/api/contacts', async (req, reply) => {
        ...
        if (search || searchDigits) {
            where = {
                OR: [
                    { contact_name: { contains: search, mode: client_1.Prisma.QueryMode.insensitive } },
                    { contact_email: { contains: search, mode: client_1.Prisma.QueryMode.insensitive } },
                    { contact_type: { contains: search, mode: client_1.Prisma.QueryMode.insensitive } },
                    { contact_notes: { contains: search, mode: client_1.Prisma.QueryMode.insensitive } },
    ...
    app.patch('/api/contacts/:id', async (req, reply) => {
        ...
        try {
            ...
            if ('phone' in req.body) {
                const digits = clamp10(toDigits(req.body.phone ?? ''));
                if (digits.length !== 10)
                    throw new Error('VALIDATION:phone must have 10 digits');
                patch.contact_phone = digits;
            }
            if ('email' in req.body)
                patch.contact_email = req.body.email ?? null;
            if ('contact_type' in req.body)
                patch.contact_type = req.body.contact_type ?? null;
            if ('notes' in req.body)
                patch.contact_notes = req.body.notes ?? null;
...
app.post('/api/admin/geocode-missing', async (_req, reply) => {
    try {
        ...
        for (let i = 0; i < props.length; i++) {
            ...
            if (i < props.length - 1)
                await sleep(1100); // be polite to OSM (≈1 req/sec)
        }
